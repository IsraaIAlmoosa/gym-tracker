...
export default async function EditInBodyPage({ params }: Props) {
  ...
  const [
    ...
    supabase.from('inbody_measurements').select(ROW_COLUMNS).eq('id', id).maybeSingle(),
  ]);

  if (profileError || rowError) {
    return <LoadErrorNotice locale={locale} />;
  }

  if (!row) {
    redirect(`/${locale}/inbody/history`);
  }

  const weightUnit = (profile?.preferred_weight_unit ?? 'kg') as WeightUnit;
  const measurement = mapInBodyRow(row as unknown as InBodyRow);
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-6 pb-24 lg:px-8 lg:py-8 lg:pb-8">
      ...
      <Card>
        <InBodyForm weightUnit={weightUnit} editing={measurement} />
      </Card>
    </div>
  );
}
